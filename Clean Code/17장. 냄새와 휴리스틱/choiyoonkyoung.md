...
>중복된 주석
- 코드만으로 충분한테 구구절절 설명하는 주석이 중복된 주석이다.

>성의 없는 주석
- 작성할 가지가 있는 주석은 잘 작성할 가치도 있다.

>주석 처리된 코드
- 코드를 읽다가 주석으로 처리도니 코드가 줄줄이 나오면 신경이 거슬린다.
- 주석으로 처리된 코드는 즉각 삭제하자
...
> 당연한 동작을 구현하지 않는다.
- 최소 놀람의 원칙에 의거해 함수난 클래스는 다른 프로그래머가 다양하게 여길 만한 동작과 기능을 제공해야 한다.
...
> 안전 절차 무시
- 실패하는 테스트 케이스는 일단 제껴두고 나중으로 미루는 태도는 신용카드가 공짜돈이라고 생각하는 것만큼 위험하다
- 컴파일러 경고를 일부 꺼버리면 빌드가 쉬워질지 모르지만 자칫하면 끝없는 디버깅에 시달린다

> 중복
- 어디서든 중복을 발견하면 없애라. 
- 가장 중요한 규칙 중 하나로 심각하게 심사숙고 해야한다.

> 추상화 수준이 올바르지 못하다.
- 추상화는 저차원 ㅅ항세 개념에서 고차원 일반 개념을 분리한다
- 추상화 수준은 거짓말이나 꼼수로 해결하지 못한다.
- 잘못된 추상화를 임시변통으로 고치는것은 불가능하다.
...
> 이름과 기능이 일치하는 함수
-   이름만으로 분명하지 않기에 구현을 살피거나 문서를 뒤적여 더 좋은 이름으로 바꾸거나 더 좋은 이름을 붙이기 쉽도록 기능     을 정리해야 한다.
-   
> 알고리즘을 이해하라
...
> 조건을 캡슐화 하라
-  bool 논리는 (if나 while 문에다 넣어 생각하지 않아도) 이해하기 어렵다.
